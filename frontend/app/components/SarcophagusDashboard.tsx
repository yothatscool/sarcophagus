...
export default function SarcophagusDashboard({ account, connex, onUserDataUpdate }: SarcophagusDashboardProps) {
  ...
  const getErrorColor = (severity: ErrorInfo['severity']) => {
    switch (severity) {
      case 'low':
        return 'bg-accent-gold/10 border-accent-gold/30 text-accent-gold';
      case 'medium':
  ...
  const getStatusIcon = () => {
    if (!userVerification?.isVerified) return '';
    if (!sarcophagusData) return '';
    if (sarcophagusData.vetAmount === '0') return '';
    return '';
  }

  if (!account) {
    return (
      <div className="bg-sarcophagus-800/50 backdrop-blur-sm border border-accent-gold/30 rounded-lg shadow-sarcophagus p-8 text-center">
        <h2 className="text-2xl font-bold text-accent-gold mb-4 font-serif">Sarcophagus Dashboard</h2>
        <p className="text-gray-300">Please connect your VeChain wallet to access the dashboard.</p>
      </div>
    );
  }

  return (
    <div className="bg-sarcophagus-800/50 backdrop-blur-sm border border-accent-gold/30 rounded-lg shadow-sarcophagus">
              ...
              <div className="flex items-center space-x-2">
                {currentError.action && (
                  <button
                    ...
                    className="px-3 py-1 bg-accent-gold hover:bg-accent-goldMedium text-primary-blue text-sm rounded font-medium"
                  >
                    {currentError.action}
                  </button>
                )}
                <button
                  ...
                  className="px-3 py-1 border border-accent-gold/30 text-accent-gold text-sm rounded hover:bg-accent-gold/10"
                >
                ...
                <div className="mt-3 pt-3 border-t border-accent-gold/20">
                  ...
                  {currentError.helpUrl && (
                    <a
                      ...
                      className="text-xs text-accent-gold hover:text-accent-goldMedium underline"
                    >
                      Get Help →
                    </a>
                  )}
      ...
      {/* Header */}
      <div className="border-b border-accent-gold/30 p-6">
        <div className="flex justify-between items-center mb-2">
          <h2 className="text-2xl font-bold text-accent-gold font-serif">Sarcophagus Dashboard</h2>
          <div className="flex items-center space-x-2">
            ...
            <button
              ...
              className={`px-3 py-1 rounded-full text-xs font-medium transition-colors ${
                isTestMode 
                  ...
                  : 'bg-vechain-green/20 text-vechain-green border border-vechain-green/40'
              }`}
            >
        ...
        {isTestMode && (
          <div className="mt-2 p-2 bg-accent-gold/10 border border-accent-gold/30 rounded text-xs text-accent-gold">
            Test Mode: Using mock data. Switch to Live Mode for real blockchain transactions.
          </div>
        )}
        {lastTransaction && (
          <div className={`mt-2 p-2 rounded text-xs ${
            lastTransaction.status === 'pending' ? 'bg-primary-blue/20 border border-primary-blue/40 text-accent-gold' :
            lastTransaction.status === 'success' ? 'bg-vechain-green/20 border border-vechain-green/40 text-vechain-green' :
      ...
      {/* Navigation Tabs */}
      <div className="border-b border-accent-gold/30">
            ...
            <button
              ...
              className={`py-4 px-1 border-b-2 font-medium text-sm ${
                activeTab === tab.id
                  ...
                  : 'border-transparent text-gray-400 hover:text-gray-300 hover:border-accent-gold/50'
              }`}
            >
              {tab.label}
            </button>
      ...
      <div className="p-6">
        ...
        <div className="bg-accent-gold/10 border border-accent-gold/30 rounded-lg p-4 mb-6">
          <h3 className="font-semibold text-accent-gold mb-2">Development Mode</h3>
        ...
        {activeTab === 'overview' && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="bg-primary-blue/20 border border-accent-gold/30 rounded-lg p-4">
                <h3 className="text-lg font-semibold text-accent-gold">Vault Balance</h3>
