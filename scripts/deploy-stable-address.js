// VeChain Native Deployment Script with Stable Address
// Uses a consistent derivation method to prevent address changes
...
async function main() {
  console.log("🏛️ Deploying with VeChain Native Tools (Stable Address)...");
  ...
  // Use a consistent derivation method
  console.log("🔐 Deriving address consistently...");
  
  // Clear any existing accounts
  web3.eth.accounts.wallet.clear();
  
  // Use a specific derivation path that we know works
  const derivationPath = "m/44'/60'/0'/0/0";
  const hdNode = web3.eth.accounts.wallet.create(1, MNEMONIC, derivationPath);
  ...
  try {
    ...
    const deploymentInfo = {
      network: "VeChain Testnet (Native Tools - Stable Address)",
    ...
    fs.writeFileSync('vechain-stable-deployment.json', JSON.stringify(deploymentInfo, null, 2));
    console.log("\n📄 Deployment info saved to: vechain-stable-deployment.json");

    // Display summary
    console.log("\n🎉 DEPLOYMENT COMPLETE!");
    console.log("==================================");
    console.log("Network: VeChain Testnet (Native Tools - Stable Address)");
