...
async function main() {
  console.log("🧪 Testing End-to-End Sarcophagus Protocol Flow\n");
  ...
  try {
    ...
    try {
      ...
      const guardians = [ethers.constants.AddressZero]; // No guardian needed for adult
      const isMinors = [false];
      const ages = [25];
      const contingentBeneficiaries = [ethers.constants.AddressZero];
      ...
      console.log("    📊 Sarcophagus data:", {
        ...
        createdAt: new Date(sarcophagusData.createdAt * 1000).toISOString(),
    ...
    try {
      ...
      const addBeneficiaryTx = await sarcophagus.addBeneficiary(
        ...
        5000, // 50%
        ethers.constants.AddressZero, // No guardian
        false, // Not minor
        30, // Age
        ethers.constants.AddressZero, // No contingent beneficiary
    ...
    try {
      ...
      // Confirm death verification (as oracle)
    ...
    try {
      ...
      const sarcophagusData = await sarcophagus.sarcophagi(deployer.address);
      if (sarcophagusData.beneficiaries.length > 0) {
    ...
    try {
      ...
      const sarcophagusData = await sarcophagus.sarcophagi(deployer.address);
      const createdAt = sarcophagusData.createdAt;
    ...
    console.log("\n💡 Next Steps:");
    console.log("1. Review any failed steps and understand the requirements");
    console.log("2. Test with different user scenarios and edge cases");
    console.log("3. Integrate with frontend for user interface testing");
    console.log("4. Consider security audit before mainnet deployment");
