// VeChain Native Deployment Script with Fixed Address
// Uses a completely stable derivation method
...
async function main() {
  console.log("🏛️ Deploying with VeChain Native Tools (Fixed Address)...");
  ...
  // Use a completely fixed approach
  console.log("🔐 Creating fixed address derivation...");
  
  // Clear any existing accounts
  web3.eth.accounts.wallet.clear();
  
  // Create a new Web3 instance for each derivation to avoid conflicts
  const web3Clean = thorify(new Web3(), VECHAIN_TESTNET_URL);
  
  // Use the most basic derivation method
  const account = web3Clean.eth.accounts.wallet.create(1, MNEMONIC)[0];
  
  // Add to the main web3 instance
  ...
  if (web3.utils.toBN(balance).lt(web3.utils.toBN(web3.utils.toWei('1', 'ether')))) {
    ...
    console.log("\n💡 This address should be stable now. Get testnet VET for this address and run the script again.");
    return;
  }
  ...
  try {
    ...
    const deploymentInfo = {
      network: "VeChain Testnet (Native Tools - Fixed Address)",
    ...
    fs.writeFileSync('vechain-fixed-deployment.json', JSON.stringify(deploymentInfo, null, 2));
    console.log("\n📄 Deployment info saved to: vechain-fixed-deployment.json");

    // Display summary
    console.log("\n🎉 DEPLOYMENT COMPLETE!");
    console.log("==================================");
    console.log("Network: VeChain Testnet (Native Tools - Fixed Address)");
